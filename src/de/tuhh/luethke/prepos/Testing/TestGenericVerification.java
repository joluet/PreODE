...
public class TestGenericVerification {
	...
	public static void main(String[] args) {
		...
		/*
		 * int UTMZoneNumber = 0; char UTMZoneLetter = ' '; LinkedList<Measurement> testData = null;
		 * if(dataFileName.endsWith(".txt")){ // San Francisco UTMZoneNumber = 10; UTMZoneLetter =
		 * 'S'; testData = CabDataParser.parseExtended(dataFileName); }else
		 * if(dataFileName.endsWith(".kml")){ // Hamburg UTMZoneNumber = 32; UTMZoneLetter = 'U';
		 * testData = LatitudeHistoryParser.parse(dataFileName); }else
		 * if(dataFileName.contains("plt")){ // Beijing UTMZoneNumber = 50; UTMZoneLetter = 'S';
		 ...
		 * List<Measurement[]> testDataVectors =
		 * PositionalTSTransformer.transformTSDataMeasurements(testData, predictionSteps, stepSize,
		 * tolerance, overallSamples);
		 * 
		 * 
		 * //Preprocessor.processData(measurements); // System.out.println(measurements.size()); //
		 * for (Measurement m : measurements) // System.out.println(m.getLat() + " " + m.getLng());
		 * 
		 * System.out.println("$-----------------------------------------------------------------");
		 * 
		 * List<SimpleMatrix> posVectors = PositionalTSTransformer.transformTSData1(testData,
		 * predictionSteps, stepSize, tolerance, overallSamples);
		 * System.out.println("Extracted "+testDataVectors
		 * .size()+" possible test- and learning-vectors.");
		...
		try {
			...
			for (int i = start + 3; i < stop; i++) {
				...
				if (i % 100 == 0) {
					...
					/*
					 * if(time > 5000 && time < 13000) { dist.mCompressionThreshold =
					 * dist.mCompressionThreshold*1.1;
					 * System.out.println("Increased compression threshold to: "
					 * +dist.mCompressionThreshold); }
					 */
					/*
					 * if(time >= 9000) { dist.mCompressionThreshold =
					 * dist.mCompressionThreshold*1.3;
					 * System.out.println("Increased compression threshold to: "
					 * +dist.mCompressionThreshold); }
					 */
				...
				/*
				 * if (i % 3 == 0) { d1 = StdRandom.gaussian(1, 0.2); d2 = StdRandom.gaussian(1,
				 * 0.2); d3 = StdRandom.gaussian(1, 0.2); d4 = StdRandom.gaussian(3, 0.2); } else if
				 * (i % 2 == 0) { d1 = StdRandom.gaussian(1, 0.2); d2 = StdRandom.gaussian(3, 0.2);
				 * d3 = StdRandom.gaussian(3, 0.2); d4 = StdRandom.gaussian(3, 0.2); } else if (i %
				 * 1 == 0) { d1 = StdRandom.gaussian(3, 0.2); d2 = StdRandom.gaussian(3, 0.2); d3 =
				 * StdRandom.gaussian(1, 0.2); d4 = StdRandom.gaussian(1, 0.2); }
				 */
		 ...
		 * i++) { SimpleMatrix m = new SimpleMatrix(6,1);
		 ...
		 * System.out.println("*******************************************"); for(int i=0; i<10;
		 * i++) { System.out.println(posVectors.get(i)); }
		 * System.out.println("********************************************");
		 */
		...
		/*
		 * double coord = 52.8; for (int i = 0; i < 100; i++) { coord += 0.03; x[i] = coord; }
		 * 
		 * coord = 9.1; for (int i = 0; i < 100; i++) { coord += 0.01; y[i] = coord; }
		 */
		...
		/*
		 * for (int i = 12; i < 999; i += 3) { long time = System.currentTimeMillis(); // z1 =
		 * calculateY1(x, y, //
		 * testData.get(i).getLat(),testData.get(i).getLng(),testData.get(i+1).getLat
		 * (),testData.get(i+1).getLng(), // dist, weightedCoordinates, false); double[][] dxVector
		 * = new double[4][1]; dxVector[0][0] = testData.get(i).getLat(); dxVector[1][0] =
		 * testData.get(i).getLng(); dxVector[2][0] = testData.get(i + 1).getLat(); dxVector[3][0] =
		 * testData.get(i + 1).getLng(); /* dxVector[4][0] = testData .get(i+2). getLat();
		 * dxVector[5][0] = testData .get(i+2). getLng();
		 */
		/*
		 * System.out.println("POS: " + testData.get(i + 2).getLat() + "," + testData.get(i +
		 * 2).getLng()); // double[][] dxVector = { { x[i] }, { y[j] } }; SimpleMatrix pointVector =
		 * new SimpleMatrix(dxVector); SimpleMatrix pointVector1 =
		 * Preprocessor.projectData4(pointVector);
		 * 
		 * dxVector = new double[2][1]; dxVector[0][0] = testData.get(i + 2).getLat();
		 * dxVector[1][0] = testData.get(i + 2).getLng(); pointVector = new SimpleMatrix(dxVector);
		 * SimpleMatrix pointVector2 = Preprocessor.projectData(pointVector);
		 * 
		 * double prob = dist.trapezoidRule(pointVector1, pointVector2, 1000, 10, 10); if (prob !=
		 * 0) System.out.println("prob: " + prob); else System.out.println(0); results[i / 3] =
		 * prob; double k = i; if (k > 1) avg = ((k - 1) / k) * avg + (1 / k) * prob; else avg =
		 * prob; System.out.println("Time: " + (System.currentTimeMillis() - time)); }
		 * 
		 * /* ExecutorService executor = Executors.newFixedThreadPool(8); ArrayList<Future<Double>>
		 * futureResults = new ArrayList<Future<Double>>(1000); for (int i = 0; i < 1000; i++) {
		 * ArrayList<Measurement> tmp = new ArrayList<Measurement>(); for (int j = i; j < (i + 3);
		 * j++) { tmp.add(testData.get(j)); } Callable<Double> worker = new TestWorker(tmp, dist);
		 * futureResults.add(executor.submit(worker)); } for (int i = 0; i < 1000; i++) { try {
		 * results[i] = futureResults.get(i).get(); System.out.println(i); double k = i; if (k > 1)
		 * avg = ((k - 1) / k) * avg + (1 / k) * results[i]; else avg = results[i]; } catch
		 ...
		 * }
		 * 
		 * System.out.println("AVG: " + avg); Arrays.sort(results); System.out.println("Median: " +
		 * results[500]);
		 */
		/*
		 * System.out.println(testData.get(10).getLat() + " " + testData.get(10).getLng() + " - " +
		 * testData.get(11).getLat() + " " + testData.get(11).getLng()); z1 = calculateY1(x, y,
		 * testData.get(10).getLat(), testData.get(10).getLng(), testData.get(11).getLat(),
		 * testData.get(11).getLng(), dist, weightedCoordinates, false); dataToFile(z1); // create
		 * your PlotPanel (you can use it as a JPanel) with a legend at // SOUTH
		 * 
		 * Plot3DPanel plot = new Plot3DPanel("SOUTH");
		 * 
		 * // add grid plot to the PlotPanel plot.addGridPlot("kernel", x, y, z1);
		 * 
	...
	/*
	 * public static double calculateY(double x, double y, SampleDist dist, ArrayList<SimpleMatrix>
	 * means) { double z = cos(x * PI) * sin(y * PI); return z; }
	 */

	/*
	 * public static double[][] calculateY1(double[] x, double[] y, double setX, double setY, double
	 * setX1, double setY1, SampleModel dist, ArrayList<SimpleMatrix> weightedCoordinates, boolean
	 * swap) { double[][] z = new double[x.length][y.length]; ExecutorService executor =
	 * Executors.newFixedThreadPool(8); ArrayList<Future<Double>> futureResults = new
	 * ArrayList<Future<Double>>(x.length * y.length); for (int i = 0; i < x.length; i++) { for (int
	 * j = 0; j < y.length; j++) { // System.out.println("x, y: "+x[i]+" - "y[j]); double[][]
	 * dxVector = new double[6][1]; dxVector[0][0] = setX; dxVector[1][0] = setY; dxVector[2][0] =
	 * setX1; dxVector[3][0] = setY1; dxVector[4][0] = 0; dxVector[5][0] = 0;
	 * 
	 * // double[][] dxVector = { { x[i] }, { y[j] } }; SimpleMatrix pointVector = new
	 * SimpleMatrix(dxVector); SimpleMatrix pointVector1 = Preprocessor.projectData6(pointVector);
	 * pointVector1.set(4, 0, x[i]); pointVector1.set(5, 0, y[j]); int[] condDim = { 0, 1, 2, 3 };
	 * Callable<Double> worker = new EvaluationWorker(pointVector1, condDim, dist);
	 * futureResults.add(executor.submit(worker)); // z[i][j] =
	 * dist.evaluateConditional(pointVector1,condDim); /* if(z[i][j] > 1E-6){
	 * System.out.println(x[i] +" " +y[j]); System.out.println(z[i][j]); }
	 */

	/*
	 * for (int k = 1; k < ((int) (z[i][j] * 100000000)); k++) { SimpleMatrix m = new
	 * SimpleMatrix(2, 1); m.set(0, 0, x[i]); m.set(1, 0, y[j]); weightedCoordinates.add(new
	 * SimpleMatrix(m)); }
	 */
	 ...
	 * } }
	 * 
	 * for (int i = 0; i < futureResults.size(); i++) { try { z[i / y.length][i % y.length] =
	 * futureResults.get(i).get(); System.out.println(i / y.length + " " + i % y.length + " = " +
	 * z[i / y.length][i % y.length]); } catch (InterruptedException | ExecutionException e) { //
	 * TODO Auto-generated catch block e.printStackTrace(); } } executor.shutdownNow();
	 * ArrayList<int[]> filtered = Postprocessor.medianFilter(z); double sum = 0; double[][]
	 * dxVector = new double[4][1]; dxVector[0][0] = setX; dxVector[1][0] = setY; dxVector[2][0] =
	 * setX1; dxVector[3][0] = setY1; SimpleMatrix pointVector = new SimpleMatrix(dxVector);
	 * SimpleMatrix pointVector1 = Preprocessor.projectData4(pointVector);
	 * 
	 * System.out.println("dist: " + (x[x.length - 1] - x[0]) + " " + (y[y.length - 1] - y[0])); for
	 * (int i = 0; i < filtered.size(); i++) { SimpleMatrix m = new SimpleMatrix(2, 1); m.set(0, 0,
	 * x[filtered.get(i)[0]]); m.set(1, 0, y[filtered.get(i)[1]]);
	 * weightedCoordinates.add(Preprocessor.projectDataBack(m)); //double prob =
	 * dist.cummulativeConditional(pointVector1, m, 400, 2, 2);
	 * //z[filtered.get(i)[0]][filtered.get(i)[1]] = prob; //sum += prob; System.out.println(sum); }
	 * System.out.println("SUM=" + sum); dataToHeatMapFile(weightedCoordinates); return z; }
	 */
